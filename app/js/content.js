...
const descriptions = {
  ...
  "What are dark patterns?": "Dark patterns are used to manipulate user's behaviour. "
};

function scrape() {
  // website has already been analyzed
  if (document.getElementById("CyberCatalyst_count")) {
    return;
  }
...
function highlight(element, type) {
  element.classList.add("CyberCatalyst-highlight");

  let body = document.createElement("span");
  body.classList.add("CyberCatalyst-highlight-body");
  ...
  if(descriptions[type]) {
    ...
    content.innerHTML = descriptions["What are dark patterns?"];
  }
...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
  if (request.message === "analyze_site") {
    ...
    let element = document.getElementById("CyberCatalyst_count");
    if (element) {
